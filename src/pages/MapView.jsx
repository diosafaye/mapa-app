...
import { supabase } from "@/lib/supabase"; // Switched from base44
import { AlertTriangle, X, Info, MapPin, ChevronRight, Flame } from "lucide-react";
...
function getCategoryEmoji(category) {
  const map = { "Church": "⛪", "Historical Landmark": "🏛️", "Natural Heritage": "🌿", "Archaeological Site": "🏺", "Museum": "🏛", "Cultural Center": "🎭" };
  return map[category] || "📍";
}
...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    // REAL-TIME: Subscribe to both sites and alerts
  ...
  return (
    <div className="relative h-full w-full flex flex-col">
      ...
      {activeAlertsList.length > 0 && (
        <div className="absolute top-0 left-0 right-0 z-[1000] space-y-2 p-4">
      ...
      >
        ...
        {/* Alert radius circles - Logic updated for Supabase data */}
        {alerts.map(alert =>
          sites
                ...
                <div className="min-w-[200px]">
                  ...
                  {alert && (
                    <div className="bg-red-50 border border-red-200 rounded p-1.5 mt-1">
                      <p className="text-xs text-red-700 font-medium text-[10px] uppercase">⚠️ {alert.severity}</p>
                      <p className="text-[10px] text-red-600">{alert.disaster_type}</p>
                    </div>
                  )}
                  <button
                    onClick={() => setSelectedSite(site)}
                    className="mt-2 text-[10px] text-primary font-black uppercase tracking-tight hover:underline flex items-center gap-1"
                  >
      ...
      <div className="absolute bottom-6 left-4 z-[1000] bg-card/95 backdrop-blur rounded-2xl border border-border p-4 shadow-2xl">
        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3">Map Legend</p>
        <div className="space-y-2">
          {Object.entries(CATEGORY_COLORS).filter(([k]) => k !== "Other").map(([cat, color]) => (
            <div key={cat} className="flex items-center gap-3">
              ...
              <span className="text-[11px] font-bold text-foreground tracking-tight">{cat}</span>
            </div>
          ))}
          <div className="flex items-center gap-3 border-t border-border pt-2 mt-2">
            <div className="w-2.5 h-2.5 rounded-full bg-destructive animate-pulse" />
            <span className="text-[11px] font-black text-destructive uppercase tracking-tighter">Under Alert</span>
          </div>
        </div>
      </div>

      <div className="absolute bottom-6 right-4 z-[1000] bg-card/95 backdrop-blur rounded-2xl border border-border px-5 py-3 shadow-2xl text-center">
        <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Active Sites</p>
        <p className="text-2xl font-black text-primary leading-none mt-1">{sites.length}</p>
      </div>
